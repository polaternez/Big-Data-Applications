...
public class RandomProducerServiceImpl implements ProducerService{
    ...
    public void init(){
        ...
        random = new Random();
        cities = Arrays.asList("İstanbul", "İzmir", "Ankara", "Antalya", "Bursa", "Van", "Elazıg",
                "Trabzon", "Aksaray", "Nigde", "Nevsehir", "Samsun", "Sinop");
        clicks = Arrays.asList("Login", "Siparislerim", "GununFirsatlari", "YurtDisiSatis",
                "Kampanyalar", "CozumMerkezi", "OkulaDonus", "EgitimCanta", "EgitimKalem", "EgitimDefter");
    ...
    public ClickRequest producer(ClickRequest request) {
        while (true) {
            ...
            request.setClick(
                    clicks.get(random.nextInt(clicks.size()))
            );
            request.setRegion(
